...
update-field-types() {
    ...
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    source_path="$script_dir/bloomerp/django_bloomerp/bloomerp/field_types/types.py"
    target_path="$script_dir/bloomerp/django_bloomerp/bloomerp/static_src/ts/modules/fieldTypes.ts"
...
field_type_class_match = re.search(r"^class FieldType\(Enum\):", source, re.MULTILINE)
if field_type_class_match is None:
    raise RuntimeError(f"Could not find FieldType in {source_path}")

field_type_class_source = source[field_type_class_match.end():]
field_types = re.findall(
    r'^    ([A-Z][A-Z0-9_]*) = FieldTypeDefinition\(\s*\n\s*id=["\']([^"\']+)["\']',
    field_type_class_source,
    re.MULTILINE,
)

if not field_types:
    raise RuntimeError(f"Could not find any field type definitions in {source_path}")
...
for member_name, field_type_id in field_types:
    lines.append(
        f"    public static readonly {member_name} = "
        f"new FieldTypeDefinition({field_type_id!r});"
    )
lines.extend(["}", ""])

target_path.write_text("\n".join(lines), encoding="utf-8")
print(f"Generated {len(field_types)} field types in {target_path}")
PY
}
